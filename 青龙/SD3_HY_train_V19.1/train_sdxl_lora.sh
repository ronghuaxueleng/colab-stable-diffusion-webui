...
# Train Model | 训练模式
model="sdxl_lora" #lora、db、sdxl_lora、sdxl_db、sdxl_cn3l、contralnet、stable_cascade_db、stable_cascade_lora、hunyuan_lora、hunyuan_db、sd3_db
...
lr_scheduler="cosine_with_min_lr" # "linear", "cosine", "cosine_with_restarts", "polynomial", "constant", "constant_with_warmup"
...
ddp_timeout=1 #ddp timeout | ddp超时，0关1开， 该参数仅限在显卡数 >= 2 使用
...
if [[ $multi_gpu == 1 ]]; then
  ...
  if [ $ddp_timeout -ne 0 ]; then
    extArgs+=("--ddp_timeout")
  fi
  if [ $ddp_gradient_as_bucket_view -ne 0 ]; then
    extArgs+=("--ddp_gradient_as_bucket_view")
  fi
  if [ $ddp_static_graph -ne 0 ]; then
    extArgs+=("--ddp_static_graph")
  fi
fi
...
if [[ $optimizer_type == "Lion" ]]; then
  ...
  extArgs+=("--optimizer_type=$optimizer_type" "--optimizer_args weight_decay=0.1 betas=.9,.95 is_paged=True")
